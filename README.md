...
- 🛡️ **抗 9074 限流** — 强制直连 + 每账号单次尝试 + 命中限流只冷却自己，不硬打
...
| 模式 | 配置 | 行为 |
| :--- | :--- | :--- |
| **全量签到**（默认） | 不设 `TRAE_ONLY` | 每轮只签 1 个账号按小时轮换；早窗自动一轮全签，9074 只冷却自己不连坐 |
...
本脚本的做法：每账号每轮只发 1 次 claim，9074 先自动换新设备号重签一次（实测有效），仍不行才冷却（不连坐），
