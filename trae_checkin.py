...
# -*- coding: utf-8 -*-
r"""
Trae SOLO 国内版 每日签到脚本（多账号版 · 抗 9074 限流）

【特性】
- 纯标准库实现，常规部署零依赖（仅解密桌面端凭据时才需 cryptography）
- 自动读取桌面端登录凭据并解密，无需手动抓包
- 多账号支持：从环境变量读取多个账号，逐个签到并汇总
- 每个账号固定一个 16 位设备号，持久化，跨运行不漂移
- 自动签到：先查今日是否已签 / 是否开放，未签才领积分并复查确认
- Token 过期自动续期，并缓存轮换后的新 refreshToken，长期不失效
- 企业微信群机器人 + PushPlus 推送（可选），运行结果自动通知
- 带图标与边框的美观日志输出

【抗 9074 限流要点】
- 强制直连，不走系统代理 / Clash —— 多账号共用出口 IP 最容易触发限流
- 只发一个小写 x-device-id —— 同时发大小写两个头会被风控判定为异常客户端
- 每账号每轮只发 1 次 claim —— 9074 是按时间窗口排队限流，当场连发
  反而不断延长自己的惩罚窗口；失败即收工，交给青龙下一轮 cron 补签
- 熔断：任一账号命中限流，本轮剩余账号直接跳过，不一起拖进惩罚窗口
- 当日状态文件 .trae_checkin_state.json —— 已签成功的账号后续运行零请求
- HTTP 429/5xx 与 code=9074 同等对待；entitlement API 查真实积分余额

【调度建议】
- 青龙 cron 高频轻量：每 30~60 分钟一次，全天多轮自然把每个账号都签上
      */30 * * * * trae_checkin.py
- 想当场多试几次设 CLAIM_TRIES=2/3（一般不需要，调大反而更容易被限）
- 只跑指定账号设 TRAE_ONLY=1（也可填 uid 或 name）
...
【推送配置（可选，留空则不推送）】
  ...
  QYWX_TOKEN        企业微信群机器人 Webhook key（机器人地址 ?key= 后面的值）
                    也兼容旧命名 WECHAT_WEBHOOK（可直接填完整 webhook 地址）
